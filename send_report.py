...
def build_detailed_report(quotes: Dict[str, QuoteSnapshot], report_date: str, messages: List[str]) -> str:
    nasdaq = quotes["Nasdaq"]
    qqq = quotes["QQQ"]
    spx = quotes["S&P 500"]
    smh = quotes["SMH"]
    ...
    lines = [
        "{0}{1}".format(REPORT_TITLE_PREFIX, report_date),
        "",
        "## 1分钟结论",
        ...
        "- 中概传导：KWEB {0}｜BABA {1}｜JD {2}｜BIDU {3}".format(
            fmt_pct(kweb.change_pct), fmt_pct(quotes["BABA"].change_pct), fmt_pct(quotes["JD"].change_pct), fmt_pct(quotes["BIDU"].change_pct)
        ),
        "- 亚洲确认：Nikkei {0}｜KOSPI {1}｜ASX 200 {2}".format(
        ...
        "- Nasdaq：{0}｜QQQ：{1}｜S&P 500：{2}｜SMH：{3}".format(
            fmt_pct(nasdaq.change_pct), fmt_pct(qqq.change_pct), fmt_pct(spx.change_pct), fmt_pct(smh.change_pct)
        ),
        "- KWEB：{0}｜BABA：{1}｜JD：{2}｜BIDU：{3}｜PDD：{4}".format(
            fmt_pct(kweb.change_pct), fmt_pct(quotes["BABA"].change_pct), fmt_pct(quotes["JD"].change_pct), fmt_pct(quotes["BIDU"].change_pct), fmt_pct(quotes["PDD"].change_pct)
        ),
        "- 日本：Nikkei {0}｜TOPIX {1}".format(fmt_pct(nikkei.change_pct), fmt_pct(topix.change_pct)),
        "- 韩国：KOSPI {0}｜KOSDAQ {1}".format(fmt_pct(kospi.change_pct), fmt_pct(kosdaq.change_pct)),
        "- 澳洲：ASX 200 {0}".format(fmt_pct(asx.change_pct)),
        "- 10Y 美债：{0}｜DXY：{1}｜USD/CNH：{2}".format(
            fmt_yield_tnx(us10y), fmt_pct(dxy.change_pct), fmt_num(usdcnh.close, 4)
        ),
        "- USD/JPY：{0}｜AUD/USD：{1}".format(fmt_num(quotes["USD/JPY"].close, 4), fmt_num(quotes["AUD/USD"].close, 4)),
        "",
        "### 3. 交易方向评分",
        "- 美股科技传导：{0}/5".format(score_us),
        "- 中概ADR支持：{0}/5".format(score_cn),
        ...
        "- 美股 → 恒科：{0}｜原因：QQQ {1}".format("正向" if score_us >= 4 else "中性/负向", fmt_pct(qqq.change_pct)),
        "- 中概 → 恒科：{0}｜原因：KWEB {1}".format("正向" if score_cn >= 4 else "中性/负向", fmt_pct(kweb.change_pct)),
        "- 亚洲 → 恒科：{0}｜原因：Nikkei {1}、KOSPI {2}、ASX {3}".format(
            "正向" if score_asia >= 4 else "中性/负向",
            fmt_pct(nikkei.change_pct), fmt_pct(kospi.change_pct), fmt_pct(asx.change_pct)
        ),
        "- 汇率 → 恒科：{0}｜原因：USD/CNH {1}".format("正向" if score_fx >= 4 else "负向", fmt_num(usdcnh.close, 4)),
        ...
        "- 阿里 / BABA：{0}".format(fmt_pct(quotes["BABA"].change_pct)),
        "- 京东 / JD：{0}".format(fmt_pct(quotes["JD"].change_pct)),
        "- 网易 / NTES：{0}".format(fmt_pct(quotes["NTES"].change_pct)),
        "- 百度 / BIDU：{0}".format(fmt_pct(quotes["BIDU"].change_pct)),
        "- 携程 / TCOM：{0}".format(fmt_pct(quotes["TCOM"].change_pct)),
        "- 哔哩哔哩 / BILI：{0}".format(fmt_pct(quotes["BILI"].change_pct)),
        "- 小鹏 / XPEV：{0}｜理想 / LI：{1}｜蔚来 / NIO：{2}".format(
            fmt_pct(quotes["XPEV"].change_pct), fmt_pct(quotes["LI"].change_pct), fmt_pct(quotes["NIO"].change_pct)
        ),
        "- 无可靠盘前代理：美团、小米、快手。",
        "",
        "### 6. 三阶段交易计划",
        ...
        "",
        "### 7. 做多 / 做空条件",
        "- 做多更有利：QQQ 与 KWEB 同向偏强，日韩澳不转弱，USD/CNH 稳定，核心 ADR 一致向上。",
        "- 做空或避险更有利：QQQ 转弱、KWEB 弱于 QQQ、USD/CNH 快速上行、日韩高开低走。",
        "",
        "### 8. 关键价位与触发器",
        "- KWEB 支撑 / 压力：{0} / {1}".format(kweb_support, kweb_resistance),
        "- QQQ 支撑 / 压力：{0} / {1}".format(qqq_support, qqq_resistance),
        "- USD/CNH 风险位：{0}".format(fmt_num(usdcnh.close * 1.002, 4) if quote_ok(usdcnh) else "暂无可靠数据"),
        "- USD/CNH 利好位：{0}".format(fmt_num(usdcnh.close * 0.998, 4) if quote_ok(usdcnh) else "暂无可靠数据"),
        "",
        "### 9. 不追单与风险提示",
        "- Nasdaq 涨但 KWEB 跌，不追多。",
        "- 美股强但 USD/CNH 快速上行，不追多。",
        "- 日本 / 韩国高开低走，警惕恒科高开低走。",
        "- 港股跳空、ADR 传导失真、流动性不足、开盘 15 分钟假突破都要防。",
        "",
        "### 10. 最终交易结论",
        "- 今日方向：{0}".format(direction),
        "- 是否适合盘前建仓：{0}".format("是" if total_score >= 20 else "只适合轻仓" if total_score >= 15 else "等确认"),
...
def wrap_html(title: str, body_text: str) -> str:
    return (
        "<!DOCTYPE html>\n<html lang=\"zh-CN\"><head><meta charset=\"utf-8\">"
        "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
        "<title>{0}</title>"
        "<style>body{{font-family:Arial,'PingFang SC',sans-serif;background:#f7f7f4;color:#222;line-height:1.6;padding:24px;max-width:960px;margin:0 auto}}"
        "article{{background:#fff;border:1px solid #e5e5df;border-radius:16px;padding:24px;box-shadow:0 10px 28px rgba(0,0,0,.05)}}"
        "h1,h2,h3{{color:#164b7a}}pre{{white-space:pre-wrap;word-break:break-word;font-family:inherit}}</style></head>"
        "<body><article><pre>{1}</pre></article></body></html>"
    ).format(html.escape(title), html.escape(body_text))


def save_outputs(report_date: str, messages: List[str], detailed_text: str, reports_dir: Path) -> Tuple[Path, Path]:
    ...
    md_path.write_text(detailed_text, encoding="utf-8")
    latest_md.write_text(detailed_text, encoding="utf-8")
    html_text = wrap_html("{0}{1}".format(REPORT_TITLE_PREFIX, report_date), detailed_text)
...
def main() -> int:
    ...
    try:
        ...
        if enough:
            ...
            detailed_text = build_detailed_report(quotes, report_date, messages)
        else:
            messages = render_insufficient(report_date, confirmed, reasons)
            detailed_text = "\n\n".join(messages).strip() + "\n"

        reports_dir = Path(args.reports_dir).expanduser()
        md_path, html_path = save_outputs(report_date, messages, detailed_text, reports_dir)
        state_path = Path(args.state_file).expanduser()
        state = load_state(state_path)
        digest = hashlib.sha256(detailed_text.encode("utf-8")).hexdigest()
